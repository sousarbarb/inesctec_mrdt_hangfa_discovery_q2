...
#include "inesctec_mrdt_localization_odom/OdomWhDiff.h"
#include "inesctec_mrdt_localization_odom/OdomWhOmni3.h"
#include "inesctec_mrdt_localization_odom/OdomWhOmni4.h"

namespace inesctec_mrdt_localization_odom
{

OdomWhROS2::OdomWhROS2() : rclcpp::Node("inesctec_mrdt_localization_odom_wh")
{
  try
  {
    readParam();
  ...
  pub_cmd_vel_ref_ =
      this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel_ref", 10);

  pub_mot_ref_ = this->create_publisher<
      inesctec_mrdt_drivers_interfaces::msg::MotRefArray>("motors_ref", 10);

  pub_odom_ = this->create_publisher<nav_msgs::msg::Odometry>("odom", 10);

  sub_mot_enc_ = this->create_subscription<
      inesctec_mrdt_drivers_interfaces::msg::MotEncArray>(
      "motors_enc", 10,
...
void OdomWhROS2::readParam()
{
  ...
  this->declare_parameter("pose_covariance");

  if (!this->has_parameter("steering_geometry"))
  {
    throw std::runtime_error(
        "[OdomWhROS2.cpp] OdomWhROS2::readParam: "
        "the node inesctec_mrdt_localization_odom requires the definition of "
        "the "
  ...
  if (this->has_parameter("pose_covariance"))
  {
    rclcpp::Parameter pose_covariance_param;
    std::vector<double> pose_covariance_tmp;

    this->get_parameter("pose_covariance", pose_covariance_param);

    pose_covariance_tmp = pose_covariance_param.as_double_array();

    if (pose_covariance_tmp.size() != 36)
    {
      throw std::runtime_error(
          "[OdomWhROS2.cpp] OdomWhROS2::readParam: "
          "pose covariance matrix must have 36 elements "
          "(x, y, z, roll, pitch, yaw)");
    }

    for (int idx = 0; idx < 36; idx++)
    {
      pose_covariance_[idx] = pose_covariance_tmp.at(idx);
    }
  }
  else
  {
  ...
  else if (steering_geometry_ == kOdomWhOmni3Str)
  {
    ...
    wh_inv[OdomWhOmni3::kWhIdxFL] =
        this->get_parameter("wh_front_left_inv").as_bool();
    wh_inv[OdomWhOmni3::kWhIdxB] = this->get_parameter("wh_back_inv").as_bool();
  ...
  else if (steering_geometry_ == kOdomWhDiffStr)
  {
    ...
    wh_d[OdomWhDiff::kWhIdxL] = this->get_parameter("wh_left_diam").as_double();
  ...
  RCLCPP_INFO(this->get_logger(), "Invert TF: %s", invert_tf_ ? "yes" : "no");

  RCLCPP_INFO(this->get_logger(), "Maximum wheel angular speed enabled: %s",
              w_ref_max_enabled_ ? "yes" : "no");

  if (w_ref_max_enabled_)
  {
    RCLCPP_INFO(this->get_logger(), "Maximum wheel angular speed: %lf (rad/s)",
                w_ref_max_);
  }

  std::stringstream pose_covariance_str;

  pose_covariance_str << "[" << std::endl;
  ...
  for (int r = 0; r < 6; r++)
  {
    ...
    for (int c = 0; c < 6; c++)
    {
      pose_covariance_str << std::setfill(' ') << std::setw(9)
                          << pose_covariance_[idx++] << " ";
    }
    pose_covariance_str << std::endl;
  }
  ...
  RCLCPP_INFO(this->get_logger(), "Pose covariance matrix:\n%s",
              pose_covariance_str.str().c_str());

  RCLCPP_INFO(this->get_logger(), "Steering geometry: %s",
              steering_geometry_.c_str());

  if (steering_geometry_ == kOdomWhOmni4Str)
  {
    RCLCPP_INFO(this->get_logger(), "  Distance front-back: %lf m",
                odom_->rob_l[OdomWhOmni4::kRobLenIdxF2B]);

    RCLCPP_INFO(this->get_logger(), "  Distance left-right: %lf m",
  ...
  {
    RCLCPP_INFO(this->get_logger(), "  Distance center-wheels: %lf m",
  ...
  {
    RCLCPP_INFO(this->get_logger(), "  Distance between wheels: %lf m",
...
void OdomWhROS2::subMotEnc(
    const inesctec_mrdt_drivers_interfaces::msg::MotEncArray::SharedPtr msg)
{
  try
  {
    for (size_t i = 0; i < msg->mot_enc.size(); i++)
    {
      odom_->setMotorDriveEncTicksDelta(i, msg->mot_enc[i].enc_delta,
    ...
    for (int idx = 0; idx < 36; idx++)
    {
      odom_msg.pose.covariance[idx] = pose_covariance_[idx];
    }
...
void OdomWhROS2::subCmdVel(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  inesctec_mrdt_drivers_interfaces::msg::MotRefArray mot_ref_msg;
