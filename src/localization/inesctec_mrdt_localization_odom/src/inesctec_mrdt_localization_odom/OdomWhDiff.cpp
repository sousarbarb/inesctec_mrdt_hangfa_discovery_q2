...
void OdomWhDiff::updateOdomDelta()
{
  ...
  odo.v_delta = 0;
  odo.w_delta =
      -(mot[kWhIdxR].dist_delta + mot[kWhIdxL].dist_delta) / rob_l[kRobLenIdx];
...
void OdomWhDiff::updateOdomDeltaInv()
{
  ...
  mot[kWhIdxR].setDistDelta(-odo.u_delta -
                            0.5 * rob_l[kRobLenIdx] * odo.w_delta);
  mot[kWhIdxL].setDistDelta(odo.u_delta -
                            0.5 * rob_l[kRobLenIdx] * odo.w_delta);
}

void OdomWhDiff::computeFwdKin(const std::vector<double> &v_mot, double &v,
                               double &vn, double &w)
{
