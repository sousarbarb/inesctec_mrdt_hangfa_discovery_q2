#include "inesctec_mrdt_localization_odom/OdomWhMotorDrive.h"
...
namespace inesctec_mrdt_localization_odom
{
  ...
  void OdomWhMotorDrive::setEncTicksDelta(const int32_t &delta_ticks)
  {
    ...
    ang_delta = inverted ? -enc_ticks_delta * 2 * M_PIf64 / ticks_per_rev : enc_ticks_delta * 2 * M_PIf64 / ticks_per_rev;
  ...
  void OdomWhMotorDrive::setDistDelta(const double &delta_dist)
  {
    ...
    enc_ticks_delta = inverted ? -std::round(ang_delta * ticks_per_rev / (2 * M_PIf64)) : std::round(ang_delta * ticks_per_rev / (2 * M_PIf64));
  }
