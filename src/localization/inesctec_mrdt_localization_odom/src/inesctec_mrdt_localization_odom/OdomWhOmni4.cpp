...
OdomWhOmni4::OdomWhOmni4(const std::vector<size_t> &wh_idx,
                         const std::vector<double> &wh_d,
                         const std::vector<bool> &wh_inv,
...
{
  mot[kWhIdxFL].setV(vel.v - vel.vn -
                     (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w *
                         0.5);
  mot[kWhIdxFR].setV(-vel.v - vel.vn -
                     (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w *
                         0.5);
  mot[kWhIdxBL].setV(vel.v + vel.vn -
                     (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w *
                         0.5);
  mot[kWhIdxBR].setV(-vel.v + vel.vn -
                     (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w *
                         0.5);
}
...
{
  mot[kWhIdxFL].setVr(vel.v_r - vel.vn_r -
                      (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w_r *
                          0.5);
  mot[kWhIdxFR].setVr(-vel.v_r - vel.vn_r -
                      (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w_r *
                          0.5);
  mot[kWhIdxBL].setVr(vel.v_r + vel.vn_r -
                      (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w_r *
                          0.5);
  mot[kWhIdxBR].setVr(-vel.v_r + vel.vn_r -
                      (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * vel.w_r *
                          0.5);
}
...
void OdomWhOmni4::updateOdomDelta()
{
  ...
  odo.u_delta = (mot[kWhIdxFL].dist_delta - mot[kWhIdxFR].dist_delta +
                 mot[kWhIdxBL].dist_delta - mot[kWhIdxBR].dist_delta) /
                4.0;
  odo.v_delta = (-mot[kWhIdxFL].dist_delta - mot[kWhIdxFR].dist_delta +
                 mot[kWhIdxBL].dist_delta + mot[kWhIdxBR].dist_delta) /
                4.0;
  odo.w_delta = -(mot[kWhIdxFL].dist_delta + mot[kWhIdxFR].dist_delta +
                  mot[kWhIdxBL].dist_delta + mot[kWhIdxBR].dist_delta) /
                (2 * (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]));
  ...
  {
    odo.x_delta = (odo.u_delta * sin(odo.w_delta) +
                   odo.v_delta * (cos(odo.w_delta) - 1)) *
                      cos(pose.th + odo.w_delta / 2.0) / odo.w_delta -
                  (odo.u_delta * (1 - cos(odo.w_delta)) +
                   odo.v_delta * sin(odo.w_delta)) *
                      sin(pose.th + odo.w_delta / 2.0) / odo.w_delta;
    odo.y_delta = (odo.u_delta * sin(odo.w_delta) +
                   odo.v_delta * (cos(odo.w_delta) - 1)) *
                      sin(pose.th + odo.w_delta / 2.0) / odo.w_delta +
                  (odo.u_delta * (1 - cos(odo.w_delta)) +
                   odo.v_delta * sin(odo.w_delta)) *
...
void OdomWhOmni4::updateOdomDeltaInv()
{
  ...
  mot[kWhIdxFL].setDistDelta(odo.u_delta - odo.v_delta -
                             (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) *
                                 odo.w_delta * 0.5);
  mot[kWhIdxFR].setDistDelta(-odo.u_delta - odo.v_delta -
                             (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) *
                                 odo.w_delta * 0.5);
  mot[kWhIdxBL].setDistDelta(odo.u_delta + odo.v_delta -
                             (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) *
                                 odo.w_delta * 0.5);
  mot[kWhIdxBR].setDistDelta(-odo.u_delta + odo.v_delta -
                             (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) *
                                 odo.w_delta * 0.5);
}

void OdomWhOmni4::computeFwdKin(const std::vector<double> &v_mot, double &v,
                                double &vn, double &w)
{
  v = (v_mot[kWhIdxFL] - v_mot[kWhIdxFR] + v_mot[kWhIdxBL] - v_mot[kWhIdxBR]) /
      4.0;
  vn =
      (-v_mot[kWhIdxFL] - v_mot[kWhIdxFR] + v_mot[kWhIdxBL] + v_mot[kWhIdxBR]) /
      4.0;
                                ...
                                const double &w, std::vector<double> &v_mot)
{
  v_mot.resize(4);
  v_mot[kWhIdxFL] =
      v - vn - (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * w * 0.5;
  v_mot[kWhIdxFR] =
      -v - vn - (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * w * 0.5;
  v_mot[kWhIdxBL] =
      v + vn - (rob_l[kRobLenIdxF2B] + rob_l[kRobLenIdxL2R]) * w * 0.5;
