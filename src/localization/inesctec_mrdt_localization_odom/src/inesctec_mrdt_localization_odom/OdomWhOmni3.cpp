...
void OdomWhOmni3::updateVelRef()
{
  ...
  vel.vn_r =
      (-mot[kWhIdxFR].v_r - mot[kWhIdxFL].v_r + 2 * mot[kWhIdxB].v_r) / 3.0;
...
{
  mot[kWhIdxFR].setV(-0.5 * sqrt(3) * vel.v - 0.5 * vel.vn -
                     rob_l[kRobLenIdx] * vel.w);
  mot[kWhIdxFL].setV(0.5 * sqrt(3) * vel.v - 0.5 * vel.vn -
                     rob_l[kRobLenIdx] * vel.w);
...
{
  mot[kWhIdxFR].setVr(-0.5 * sqrt(3) * vel.v_r - 0.5 * vel.vn_r -
                      rob_l[kRobLenIdx] * vel.w_r);
  mot[kWhIdxFL].setVr(0.5 * sqrt(3) * vel.v_r - 0.5 * vel.vn_r -
                      rob_l[kRobLenIdx] * vel.w_r);
...
void OdomWhOmni3::updateOdomDelta()
{
  ...
  odo.u_delta =
      sqrt(3) * (-mot[kWhIdxFR].dist_delta + mot[kWhIdxFL].dist_delta) / 3.0;
  odo.v_delta = (-mot[kWhIdxFR].dist_delta - mot[kWhIdxFL].dist_delta +
                 2 * mot[kWhIdxB].dist_delta) /
                3.0;
  odo.w_delta = -(mot[kWhIdxFR].dist_delta + mot[kWhIdxFL].dist_delta +
                  mot[kWhIdxB].dist_delta) /
                (3 * rob_l[kRobLenIdx]);
...
void OdomWhOmni3::updateOdomDeltaInv()
{
  ...
  mot[kWhIdxFR].setDistDelta(-0.5 * sqrt(3) * odo.u_delta - 0.5 * odo.v_delta -
                             rob_l[kRobLenIdx] * odo.w_delta);
  mot[kWhIdxFL].setDistDelta(0.5 * sqrt(3) * odo.u_delta - 0.5 * odo.v_delta -
                             rob_l[kRobLenIdx] * odo.w_delta);
  mot[kWhIdxB].setDistDelta(odo.v_delta - rob_l[kRobLenIdx] * odo.w_delta);
}

void OdomWhOmni3::computeFwdKin(const std::vector<double> &v_mot, double &v,
                                double &vn, double &w)
{
...
{
  v_mot.resize(3);
  v_mot[kWhIdxFR] = -0.5 * sqrt(3) * v - 0.5 * vn - rob_l[kRobLenIdx] * w;
  v_mot[kWhIdxFL] = 0.5 * sqrt(3) * v - 0.5 * vn - rob_l[kRobLenIdx] * w;
