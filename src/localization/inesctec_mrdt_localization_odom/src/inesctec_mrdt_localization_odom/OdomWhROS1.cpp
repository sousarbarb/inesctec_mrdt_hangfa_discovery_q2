...
OdomWhROS1::OdomWhROS1() : nh_priv_("~")
{
  ...
  pub_mot_ref_ =
      nh_.advertise<inesctec_mrdt_drivers_interfaces::MotRefArrayROS1>(
          "motors_ref", 10);

  pub_odom_ = nh_.advertise<nav_msgs::Odometry>("odom", 10);

  sub_mot_enc_ =
      nh_.subscribe<inesctec_mrdt_drivers_interfaces::MotEncArrayROS1>(
...
void OdomWhROS1::readParam()
{
  ...
  nh_priv_.param<std::string>("base_frame_id", base_frame_id_,
                              "base_footprint");

  nh_priv_.param<std::string>("odom_frame_id", odom_frame_id_, "odom");
  ...
  if (nh_priv_.hasParam("pose_covariance"))
  {
    std::vector<float> pose_covariance_tmp;

    nh_priv_.getParam("pose_covariance", pose_covariance_tmp);
  ...
  else if (steering_geometry_ == kOdomWhOmni3Str)
  {
    if (!nh_priv_.hasParam("rob_dist_center_wh") ||
        ...
        !nh_priv_.hasParam("wh_back_idx") || !nh_priv_.hasParam("wh_back_inv"))
    {
      throw std::runtime_error(
          "[OdomWhROS1.cpp] OdomWhROS1::readParam: "
          "the steering geometry " +
  ...
  else if (steering_geometry_ == kOdomWhDiffStr)
  {
    if (!nh_priv_.hasParam("rob_dist_between_wh") ||
        ...
        !nh_priv_.hasParam("wh_left_idx") || !nh_priv_.hasParam("wh_left_inv"))
    {
      throw std::runtime_error(
          "[OdomWhROS1.cpp] OdomWhROS1::readParam: "
          "the steering geometry " +
  ...
  ROS_INFO("[%s] Base frame ID: %s", ros::this_node::getName().c_str(),
           base_frame_id_.c_str());

  ROS_INFO("[%s] Odom frame ID: %s", ros::this_node::getName().c_str(),
           odom_frame_id_.c_str());

  ROS_INFO("[%s] Publish TF: %s", ros::this_node::getName().c_str(),
           publish_tf_ ? "yes" : "no");

  ROS_INFO("[%s] Invert TF: %s", ros::this_node::getName().c_str(),
           invert_tf_ ? "yes" : "no");
  ...
  ROS_INFO("[%s] Pose covariance matrix:\n%s",
           ros::this_node::getName().c_str(),
           pose_covariance_str.str().c_str());

  ROS_INFO("[%s] Steering geometry: %s", ros::this_node::getName().c_str(),
  ...
  else if (steering_geometry_ == kOdomWhDiffStr)
  {
    ...
    ROS_INFO(
        ...
        odom_->getMotorDriveIdxStr(1).c_str());
    ROS_INFO(
        "[%s] Wheel diameters (R L): "
        "[%lf %lf] m",
        ros::this_node::getName().c_str(), odom_->mot[OdomWhDiff::kWhIdxR].wh_d,
        odom_->mot[OdomWhDiff::kWhIdxL].wh_d);
    ROS_INFO(
        "[%s] Wheel inverted (R L): "
        "[%d %d] (0|1)",
        ros::this_node::getName().c_str(),
...
void OdomWhROS1::subCmdVel(const geometry_msgs::Twist::ConstPtr &msg)
{
  ...
  inesctec_mrdt_drivers_interfaces::MotRefArrayROS1 mot_ref_msg;
