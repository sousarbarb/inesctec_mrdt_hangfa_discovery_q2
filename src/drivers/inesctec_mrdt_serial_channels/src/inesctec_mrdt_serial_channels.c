...
char ChannelDataToSendBuffer_[COMMUNICATION_CHANNELS_MAX_SIZE + 1 +
                              1];  // one for channel identification and another
                                   // for /0
...
void AddAddrsToChannelsValuesAddrTable(char channel, unsigned char *addr,
                                       unsigned char size)
{
  ...
  if (CheckChannel(channel))
  {
    ...
    {
      ChannelsValuesAddrTable2_[channelAux] = (unsigned char **) realloc(
          ChannelsValuesAddrTable2_[channelAux],
          (ChannelsSize_[channelAux] + size) * sizeof(unsigned char *));
    ...
    for (i = 0; i < size; ++i)
    {
      if (*psample)
      {  // bigEndianTest
        (ChannelsValuesAddrTable2_[channelAux])[ChannelsSize_[channelAux]] =
            addr + i;
      }
      else
      {
...
{
  AddAddrsToChannelsValuesAddrTable(channel, (unsigned char *) value_addr,
                                    sizeof(int8_t));
}
