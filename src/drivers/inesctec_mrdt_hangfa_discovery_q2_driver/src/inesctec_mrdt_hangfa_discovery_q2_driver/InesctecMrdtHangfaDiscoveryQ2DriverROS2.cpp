...
InesctecMrdtHangfaDiscoveryQ2DriverROS2::
    InesctecMrdtHangfaDiscoveryQ2DriverROS2()
    : Node("inesctec_mrdt_hangfa_discovery_q2_driver"),
      serial_comms_first_fault_(true)
{
  ...
  pub_mot_enc_ = this->create_publisher<
      inesctec_mrdt_drivers_interfaces::msg::MotEncArray>("motors_enc", 10);

  sub_mot_ref_ = this->create_subscription<
      inesctec_mrdt_drivers_interfaces::msg::MotRefArray>(
      "motors_ref", 1,
  ...
  serial_port_timer_ = this->create_wall_timer(
      1s, std::bind(&InesctecMrdtHangfaDiscoveryQ2DriverROS2::checkSerialComms,
                    this));
}
...
void InesctecMrdtHangfaDiscoveryQ2DriverROS2::getParam()
{
  ...
  RCLCPP_INFO(this->get_logger(), "Encoder resolution: %lf (ticks/rev)",
              rob_.mot[0].encoder_res);

  RCLCPP_INFO(this->get_logger(), "Gear reduction ratio: %lf (n:1)",
              rob_.mot[0].gear_reduction);

  RCLCPP_INFO(this->get_logger(), "Serial port: %s", serial_port_name_.c_str());
}
...
void InesctecMrdtHangfaDiscoveryQ2DriverROS2::checkSerialComms()
{
  if (!rob_.isSerialOpen())
  {
    ...
    {
      serial_comms_first_fault_ = true;
      RCLCPP_INFO(this->get_logger(), "Opened serial port %s.",
                  serial_port_name_.c_str());
