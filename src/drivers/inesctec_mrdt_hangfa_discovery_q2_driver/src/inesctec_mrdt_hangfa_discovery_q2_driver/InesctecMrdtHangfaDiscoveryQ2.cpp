...
void Motor::setWr(const double &w_ref) { w_r = w_ref; }
...
InesctecMrdtHangfaDiscoveryQ2::InesctecMrdtHangfaDiscoveryQ2(
    ...
    : serial_port_name_(std::move(serial_port_name)), serial_async_(nullptr)
{
  serial_cfg_ = InitCommunications();
}
...
bool InesctecMrdtHangfaDiscoveryQ2::openSerial(const bool dbg)
{
  try
  {
    serial_async_ = new CallbackAsyncSerial(
        ...
        boost::asio::serial_port_base::parity(kSerialParity), kSerialDataBits,
  ...
  catch (boost::system::system_error &e)
  {
    ...
    if (dbg)
    {
      std::cerr << "[InesctecMrdtHangfaDiscoveryQ2.cpp] "
                   "InesctecMrdtHangfaDiscoveryQ2::openSerial: "
...
void InesctecMrdtHangfaDiscoveryQ2::closeSerial(const bool dbg)
{
  if (serial_async_)
  {
    ...
    catch (...)
    {
      ...
      {
        std::cerr << "[InesctecMrdtHangfaDiscoveryQ2.cpp] "
                     "InesctecMrdtHangfaDiscoveryQ2::closeSerial: "
...
void InesctecMrdtHangfaDiscoveryQ2::setSerialPortName(
...
void InesctecMrdtHangfaDiscoveryQ2::rcvSerialData(const char *data,
                                                  unsigned int len)
{
