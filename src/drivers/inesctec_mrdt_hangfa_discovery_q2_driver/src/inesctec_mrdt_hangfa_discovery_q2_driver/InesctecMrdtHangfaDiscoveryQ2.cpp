...
namespace inesctec_mrdt_hangfa_discovery_q2_driver
{
  ...
  void Motor::setEncoderRes(const double &enc_res)
  {
    if (enc_res <= 0)
    {
      throw std::invalid_argument(
          "[InesctecMrdtHangfaDiscoveryQ2.cpp] Motor::setEncoderRes: "
  ...
  void Motor::setGearReduction(const double &gear_ratio)
  {
    ...
    {
      throw std::invalid_argument(
          "[InesctecMrdtHangfaDiscoveryQ2.cpp] Motor::setGearReduction: "
  ...
  InesctecMrdtHangfaDiscoveryQ2::InesctecMrdtHangfaDiscoveryQ2()
      : serial_port_name_("/dev/ttyACM0"), serial_async_(nullptr)
  {
    serial_cfg_ = InitCommunications();
  }

  InesctecMrdtHangfaDiscoveryQ2::InesctecMrdtHangfaDiscoveryQ2(std::string serial_port_name)
  ...
  void InesctecMrdtHangfaDiscoveryQ2::init()
  {
    reset();
    if (isSerialOpen())
    {
  ...
  bool InesctecMrdtHangfaDiscoveryQ2::openSerial(const bool dbg)
  {
    try
    {
      ...
      serial_async_->setCallback(
          std::bind(&InesctecMrdtHangfaDiscoveryQ2::rcvSerialData, this,
    ...
    catch (boost::system::system_error &e)
    {
      ...
      if (dbg)
      {
        std::cerr << "[InesctecMrdtHangfaDiscoveryQ2.cpp] InesctecMrdtHangfaDiscoveryQ2::openSerial: "
  ...
  void InesctecMrdtHangfaDiscoveryQ2::closeSerial(const bool dbg)
  {
    if (serial_async_)
    {
      try
      ...
      catch (...)
      {
        if (dbg)
        {
          std::cerr << "[InesctecMrdtHangfaDiscoveryQ2.cpp] InesctecMrdtHangfaDiscoveryQ2::closeSerial: "
  ...
  bool InesctecMrdtHangfaDiscoveryQ2::isSerialOpen()
  {
    return serial_async_ ? !(serial_async_->errorStatus() || (!serial_async_->isOpen())) : false;
  }

  void InesctecMrdtHangfaDiscoveryQ2::setSerialPortName(const std::string &serial_port_name)
  {
    serial_port_name_ = serial_port_name;
  }

  void InesctecMrdtHangfaDiscoveryQ2::reset()
  {
    for (auto &m : mot)
    {
      m.reset();
  ...
  void InesctecMrdtHangfaDiscoveryQ2::stopMotors()
  {
    for (auto &m : mot)
    {
      m.w_r = 0;
  ...
  void InesctecMrdtHangfaDiscoveryQ2::rcvSerialData(const char *data, unsigned int len)
  {
  ...
  void InesctecMrdtHangfaDiscoveryQ2::sendSerialData()
  {
    mtx_.lock();
